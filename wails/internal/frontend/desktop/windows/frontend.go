...
func NewFrontend(ctx context.Context, appoptions *options.App, myLogger *logger.Logger, appBindings *binding.Bindings, dispatcher frontend.Dispatcher) *Frontend {
	...
	println("Created frontend result")
	...
	if _starturl, _ := ctx.Value("starturl").(*url.URL); _starturl != nil {
		...
		println("Start URL ", result.startURL.String())
		return result
	}
...
func (f *Frontend) Run(ctx context.Context) error {
	...
	if f.frontendOptions.WaitForParent && f.frontendOptions.Windows.ParentWindow == 0 {
		if f.frontendOptions.OnContext != nil {
			go func() {
				f.frontendOptions.OnContext(f.ctx)
			}()
		}
	...
	mainWindow := NewWindow(f.frontendOptions, f.versionInfo, f.chromium)
...
func (f *Frontend) CreateWindow(parent uintptr) {
	runtime.LockOSThread()
	defer runtime.UnlockOSThread()
	f.frontendOptions.Windows.ParentWindow = parent
	f.Run(f.ctx)
}
...
func (f *Frontend) Quit() {
	...
	f.mainWindow.Invoke(winc.Exit, false)
}

func (f *Frontend) Invoke(fn func()) {
	f.mainWindow.Invoke(fn, true)
}
...
func (f *Frontend) setupChromium() {
	...
	chromium.AdditionalBrowserArgs = append(chromium.AdditionalBrowserArgs, "--enable-features=OverlayScrollbar,OverlayScrollbarWinStyle,OverlayScrollbarWinStyleAnimation")
	...
	chromium.ProcessFailedCallback = func(sender *edge.ICoreWebView2, args *edge.ICoreWebView2ProcessFailedEventArgs) {
		...
		f.logger.Error("WebView2Process failed with kind %d", kind)
		switch kind {
	...
	println("Embedding chromium")
	chromium.Embed(f.mainWindow.Handle())
	println("Embedded chrome")
	...
	if f.frontendOptions.Debug.OpenInspectorOnStartup {
		chromium.OpenDevToolsWindow()
	}
	...
	println("Current URL", f.startURL.String())
...
func (f *Frontend) processMessage(message string) {
	if message == "drag" {
		fmt.Printf("Got drag event from window\n")
	...
	println("Dispatching message as goroutine", message)
...
func (f *Frontend) processMessageWithAdditionalObjects(message string, sender *edge.ICoreWebView2, args *edge.ICoreWebView2WebMessageReceivedEventArgs) {
	if strings.HasPrefix(message, "file:drop") {
		...
		println("Dispatching with addtional values as goroutine")
...
func (f *Frontend) startDrag() error {
	...
	if f.frontendOptions.OnDrag != nil {
		f.frontendOptions.OnDrag()
	}
	return nil
}
