...
func NewWindow(appoptions *options.App, versionInfo *operatingsystem.WindowsVersionInfo, chromium *edge.Chromium) *Window {
	...
	if appoptions.Windows != nil && appoptions.Windows.LayeredWindow {
		println("Window is layered ", result.framelessWithDecorations)
	...
	winc.RegisterClass(windowClassName, winc.GeneralWndprocCallBack)
	var parent uintptr

	if appoptions.Windows != nil && appoptions.Windows.LayeredWindow {
		parent = appoptions.Windows.ParentWindow
		dwStyle = w32.WS_CHILDWINDOW
	}

	runtime.LockOSThread()
	handle := winc.CreateWindow(windowClassName, parent, uint(exStyle), uint(dwStyle))

	println("Created window", "exStyle ", uint(exStyle), "Style ", uint(dwStyle))
	if appoptions.Windows != nil && appoptions.Windows.LayeredWindow {
		win32.SetLayeredWindowAttributes(handle, 0, 0, 0x00000001)
	}
...
func (w *Window) WndProc(msg uint32, wparam, lparam uintptr) uintptr {
	...
	case w32.WM_MOVE, w32.WM_MOVING:
		...
		if w.frontendOptions.OnDrag != nil {
			w.frontendOptions.OnDrag()
		}
